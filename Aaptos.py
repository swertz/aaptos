...
class serverThread(Thread):
    def __init__(
        self, group=None, target=None, name=None, args=(), kwargs={}, daemon=False
    ):
        Thread.__init__(
    ...
    def run(self):
        server = AaptosSOAP.SOAPServer(
            (AaptosSettings.SOAPServer, AaptosSettings.SOAPPort)
        )
...
class cliThread(Thread):
    ...
    def run(self):
        ...
        cli_app = AaptosCli.MyAaptosCliApp(
            soapProxy=aaptos, loggerEnabled=self.loggerEnabled
        )
        cli_app.run()
...
class AaptosDaemon(Daemon):
    ...
    def run(self):
        ...
        loggerEnabled.set()
        serverThread(
            name="SOAPServer", daemon=False, kwargs={"serverRunning": serverRunning}
        ).start()
        if self.withDb:
            loggerThread(
                name="DBLogger",
                daemon=True,
                kwargs={"serverRunning": serverRunning, "loggerEnabled": loggerEnabled},
            ).start()
...
It can be started either as a deamon, or interactively, with or without db and cli components."""
    ...
    if options.daemon:
        if not args or args[-1] not in ["start", "stop", "restart"]:
            parser.error(
                "Daemon mode requires an additional start/stop/restart argument."
            )
        else:
            mode = args[-1]
    else:
        if args and args[-1] in ["start", "stop", "restart"]:
            parser.error(
                "%s is intended for daemon mode. Use together with --daemon" % args[-1]
            )
    if options.withCli:
        if options.daemon:
    ...
    if options.daemon:
        ...
        if daemon.getpid():
            sys.stderr.write(
                "pidfile %s already exist. Already running as a daemon?\n" % pidfile
            )
            sys.exit(1)
        serverRunning = Event()
        loggerEnabled = Event()
        loggerEnabled.set()
        s = serverThread(
            name="SOAPServer", daemon=True, kwargs={"serverRunning": serverRunning}
        )
        if options.withDb:
            loggerThread(
                name="DBLogger",

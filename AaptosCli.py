...
class GaugeWidget(npyscreen.Slider):
    ...
    def set_value(self, val):
        ...
        if self.__value > self.out_of:
            raise ValueError(
                "GaugeWidget value out of bound: %f > %f" % (self.__value, self.out_of)
            )
...
class MyAaptosCliApp(npyscreen.NPSAppManaged):
    ...
    def onStart(self):
        ...
        if self.soapProxy is None:
            self.soapProxy = AaptosSOAP.SOAPProxy(
                "http://%s:%d/" % (AaptosSettings.SOAPServer, AaptosSettings.SOAPPort)
            )
        # the forms
        self.addForm(
            "MAIN", MainForm, name="Welcome to Aaptos", minimum_lines=20, columns=108
        )
        for instr in self.instruments():
            self.addFormClass(
                "SETTINGS%s" % instr,
                SettingsForm,
                name="%s Settings" % instr,
                minimum_lines=20,
                columns=108,
            )
...
class SettingsForm(npyscreen.ActionForm):
    def create(self):
        ...
        logging.debug(
            "creating SettingsForm with name %s => psunit = %s" % (self.name, psunit)
        )
        try:
        ...
        self.currentLimit = self.add(
            npyscreen.TitleText, name="Current (A):", value=str(I)
        )
        self.nextrely += 2
        ...
        try:
            ...
            logging.debug(
                "got levels from device: %s" % str((voltMin, voltMax, curMin, curMax))
            )
        except socket.error:
            curMin = levels[1][0]
            curMax = levels[1][3]
        ...
        self.voltageWarning = self.add(
            TitleGauge,
            name="Voltage Warning Level",
            unit="V",
            step=0.1,
            levels=(voltMin, voltMin, voltMax, voltMax),
            value=levels[0][1],
        )
        self.voltageDanger = self.add(
            TitleGauge,
            name="Voltage Danger Level",
            unit="V",
            step=0.1,
            levels=(voltMin, voltMin, voltMin, voltMax),
            value=levels[0][2],
        )
        self.currentWarning = self.add(
            TitleGauge,
            name="Current Warning Level",
            unit="A",
            step=0.01,
            levels=(curMin, curMin, curMax, curMax),
            value=levels[1][1],
        )
        self.currentDanger = self.add(
            TitleGauge,
            name="Current Danger Level",
            unit="A",
            step=0.01,
            levels=(curMin, curMin, curMin, curMax),
            value=levels[1][2],
        )
    ...
    def on_ok(self):
        ...
        getattr(self.parentApp.getForm("MAIN"), psunit).set_levels(
            [
                (
                    volt_tmp[0],
                    self.voltageWarning.value,
                    self.voltageDanger.value,
                    volt_tmp[3],
                ),
                (
                    curr_tmp[0],
                    self.currentWarning.value,
                    self.currentDanger.value,
                    curr_tmp[3],
                ),
            ]
        )
        ...
        aaptos = self.parentApp.soapProxy
        try:
            aaptos.configureInstrument(
                psunit, V=float(self.voltage.value), I=float(self.currentLimit.value)
            )
            ...
            npyscreen.notify_wait(
                e.message, title="Error", form_color="STANDOUT", wrap=True, wide=False
            )
        else:
            self.editing = False
            self.parentApp.switchFormPrevious()
...
class MainForm(npyscreen.FormBaseNewWithMenus):
    ...
    def update_fields(self):
        ...
        try:
            ...
            values = (
                aaptos.getStatus()
            )  # TODO: this is "very" slow and should run in the background. update_fields should just pick the last readings
            # values = self.values
    ...
    def check_errors(self):
        ...
        try:
            for device, errors in aaptos.getErrors().items():
                errorMessages = [
                    "[Errno %s] %s" % (error[0], error[1]) for error in errors
                ]
                if len(errorMessages):
                    message = "\n".join(errorMessages)
                    npyscreen.notify_confirm(
                        message, title="Error on device %s" % device, editw=1
                    )
        except socket.error:
            self.setStatus(False)
        else:
    ...
    def getDefaultLevels(self, instrument):
        ...
        return [
            (
                voltMin,
                (voltMax - voltMin) / 3.0,
                2.0 * (voltMax - voltMin) / 3.0,
                voltMax,
            ),
            (curMin, (curMax - curMin) / 3.0, 2 * (curMax - curMin) / 3.0, curMax),
        ]
    ...
    def create(self):
        ...
        self.date_widget = self.add(
            npyscreen.FixedText,
            value=datetime.now().strftime("%b %d %Y %H:%M:%S"),
            editable=False,
        )
        self.nextrely -= 1
        ...
        for instr, vals in values.items():
            setattr(
                ...
                self.add(
                    PowerBox,
                    name=instr,
                    levels=self.getDefaultLevels(instr),
                    values=vals,
                    width=50,
                    height=4,
                    relx=relx,
        ...
        self.enablePower = self.add(
            activeCheckBox, value=False, name="Enabled", width=50
        )
        self.remoteLock = self.add(
            activeCheckBox, value=False, name="Lock front panel", width=50
        )
        self.hideDisplay = self.add(
            activeCheckBox, value=True, name="Hide device displays", width=50
        )
        self.dblog = self.add(
            activeFormControlCheckbox,
            value=False,
            name="Log values",
            color="NO_EDIT",
            width=50,
        )
        self.lograte = self.add(
            activeTitleSlider, width=50, relx=5, lowest=1, out_of=60, name="Period"
        )
        ...
        self.menu.addItemsFromList(
            [("Recall", self.do_recall, "^R"), ("Save", self.do_save, "^S")]
        )
        self.m1s1 = self.menu.addNewSubmenu("Settings", "^E")
        for instr in self.parentApp.instruments():
            self.m1s1.addItem(
                "%s settings" % instr, self.do_settings, None, None, (instr,)
            )

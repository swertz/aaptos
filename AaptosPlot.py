...
def main_live(bufferDepth, pollingTime):
    ...
    fig.subplots_adjust(
        left=0.075, bottom=0.10, right=0.95, top=0.95, wspace=0.175, hspace=0.30
    )
    ...
    while True:
        ...
        for index, (device, values) in enumerate(status.items()):
            ...
            plt.ylim(
                (
                    np.amin(v) - 0.1 * abs(np.amin(v) - np.amax(v)),
                    np.amax(v) + 0.1 * abs(np.amin(v) - np.amax(v)),
                )
            )
            ...
            plt.ylim(
                (
                    np.amin(c) - 0.1 * abs(np.amin(c) - np.amax(c)),
                    np.amax(c) + 0.1 * abs(np.amin(c) - np.amax(c)),
                )
            )
            plt.xlim((np.amin(t), np.amax(t)))
        plt.draw()
...
Support for both live stream (from the SOAP server) or database inspection."""
    ...
    parser.add_option(
        ...
        type="int",
        dest="pollingTime",
        default=AaptosSettings.PoolDelay,
